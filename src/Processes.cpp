...
int Processes::Executor()
{
    int temp = 0;
    for(unsigned i = 0; i < currCmds.size(); ++i)
    {
        temp = currCmds.at(i)->Executor();
    }
    this->DeleteInput();
    return temp;
}
    ...
    if(count(input.begin(), input.end(), ';') >= 1)
    {
        while(input.find(";") != string::npos)
        {
            ...
            if(detected && !parenthesis)
            {
                ...
                vector<string> lhsCommand;
                unsigned i = 0;
                while(currCs.at(i) != "&&" && currCs.at(i) != "||")
                {
                    lhsCommand.push_back(currCs.at(i));
                    ++i;
                }
                prevConnector = currCs.at(i);
                Base * temp3;
                if(lhsCommand.at(0) == "test")
                {
                    temp3 = new Test(lhsCommand);
                }
                else
                {
                    temp3 = new Cmd(lhsCommand);
                }
